...
var bigfiveDescriptions = [];
bigfiveDescriptions['Openness'] = 'Appreciation for art, emotion, adventure, unusual ideas, curiosity, and variety of experience.';
bigfiveDescriptions['Conscientiousness'] = 'A tendency to be organized and dependable, show self - discipline, act dutifully, aim for achievement.';
bigfiveDescriptions['Extraversion'] = 'Energy, positive emotions, surgency, assertiveness, sociability.';
bigfiveDescriptions['Agreeableness'] = 'A tendency to be compassionate and cooperative rather than suspicious and antagonistic towards others';
bigfiveDescriptions['Neuroticism'] = 'The tendency to experience unpleasant emotions easily, such as anger, anxiety, depression, and vulnerability.'

function drawChart() {
  google.charts.load('43', {
    'packages': ['sankey', 'corechart', 'bar']
  });
  google.charts.setOnLoadCallback(personaData, false);
}
...
function readPersonaData(pdata) {
  ...
  function newTweet(id, chartdata) {
    ...
    var contentString = bigfiveDescriptions[key];
  ...
  var data;
  generated_combined.forEach(function (pi) {
    if (pi.data.id == persona) {
      data = pi.data;
    }
  });
  ...
  var bigfive = data.tree.children[0].children[0].children;
